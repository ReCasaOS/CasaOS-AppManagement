...
- **The module path is `github.com/inkly/CasaOS-AppManagement`.** The Go module and every import of it moved off `github.com/IceWhaleTech`, and the shared library is now `github.com/inkly/CasaOS-Common v0.4.22`, so logs, stack traces and `go version -m` on the shipped binary name this distribution rather than upstream; App Store catalogue and CDN icon URLs still point at IceWhale's repositories and are unchanged.
