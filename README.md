...
- **A retention for backups somebody asks for, and a copy of the catalogue** (v0.4.50). `POST /compose/{id}/backup` takes `keep`: once the backup has landed, older backups of that app at that destination beyond that number are deleted, the same arithmetic a schedule's retention uses. Absent keeps everything, as before. And the App Store's catalogue, a zip of `IceWhaleTech/_appstore` that nobody maintains, is fetched from this distribution's nightly copy of it (`ReCasaOS/_appstore`) when the original stops answering; the configured URL is not changed, and the original is tried first on every update.
